  ...
  const filterCategories = [
    {
      title: 'Valuation',
      filters: [
        { key: 'marketCap', name: 'Market Cap (M)', unit: 'M', range: [100, 100000] },
    ...
    },
    {
      title: 'Dividends',
      filters: [
        { key: 'dividendYield', name: 'Dividend Yield (%)', unit: '%', range: [0, 15] },
        { key: 'payoutRatio', name: 'Payout Ratio (%)', unit: '%', range: [0, 100] },
      ]
    },
    {
      title: 'Technical',
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: theme.colors.surface, borderBottomColor: theme.colors.border }]}>
        ...
        <Text style={[styles.headerTitle, { color: theme.colors.text }]}>Advanced Screener</Text>
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.presetsContainer}>
            {[
              ...
              <TouchableOpacity key={index} style={[styles.presetChip, { borderColor: preset.color }]}>
                <Text style={[styles.presetText, { color: preset.color }]}>{preset.name}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
        </View>
        ...
        {filterCategories.map((category, categoryIndex) => (
          <View key={categoryIndex} style={styles.section}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>{category.title}</Text>
            {category.filters.map((filter, filterIndex) => (
              <View key={filterIndex} style={[styles.filterCard, { backgroundColor: theme.colors.surface }]}>
                <View style={styles.filterHeader}>
                  <Text style={[styles.filterName, { color: theme.colors.text }]}>{filter.name}</Text>
                  <Switch
                    ...
                    thumbColor={filters[filter.key]?.enabled ? '#FFFFFF' : theme.colors.textTertiary}
                  />
                </View>
                    ...
                    <View style={styles.rangeInputs}>
                      <View style={[styles.inputContainer, { backgroundColor: theme.colors.background, borderColor: theme.colors.border }]}>
                        ...
                        <Text style={[styles.inputValue, { color: theme.colors.text }]}>
                          {filters[filter.key]?.min || filter.range[0]}{filter.unit}
                        </Text>
                      </View>
                      <View style={[styles.inputContainer, { backgroundColor: theme.colors.background, borderColor: theme.colors.border }]}>
                        <Text style={[styles.inputLabel, { color: theme.colors.textSecondary }]}>Max</Text>
                        <Text style={[styles.inputValue, { color: theme.colors.text }]}>
                          {filters[filter.key]?.max || filter.range[1]}{filter.unit}
                        </Text>
                      </View>
                    </View>
                    <View style={[styles.rangeSlider, { backgroundColor: theme.colors.surfaceSecondary }]}>
                      <View style={[styles.rangeTrack, { backgroundColor: theme.colors.primary }]} />
                    </View>
                  </View>
                )}
              </View>
        ...
        <View style={styles.section}>
          <LinearGradient
            ...
            style={[styles.resultsPreview, { borderColor: theme.colors.border }]}
          >
            <Text style={[styles.resultsTitle, { color: theme.colors.text }]}>Estimated Results</Text>
            <Text style={[styles.resultsCount, { color: theme.colors.primary }]}>~247 stocks match your criteria</Text>
            <Text style={[styles.resultsDescription, { color: theme.colors.textSecondary }]}>
      ...
      <View style={[styles.bottomActions, { backgroundColor: theme.colors.surface, borderTopColor: theme.colors.border }]}>
        <TouchableOpacity style={[styles.saveButton, { backgroundColor: theme.colors.surfaceSecondary }]}>
          <Save size={20} color={theme.colors.textSecondary} />
          <Text style={[styles.saveButtonText, { color: theme.colors.textSecondary }]}>Save</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[styles.runButton, { backgroundColor: theme.colors.primary }]} onPress={runScreener}>
